...
4. **Tested.** Promote→demote round-trip returns the row to `member`; demoting the last admin is blocked (`LastAdminError`); demoting a global admin is blocked (`GlobalAdminError`); per-org only (a user who is admin of two orgs keeps their role in the other org); non-admin caller → 403; the frontend "Make member" calls `demoteAdmin`.
...
- [ ] **Task 3 — Frontend wiring (AC: #3)**
  ...
  - [ ] `frontend/src/pages/MembersPage.tsx`: on `admin`-role rows, render a "Make member" button beside "Remove"; wire `handleDemote(userId)` → `demoteAdmin`. Map the `code` from the error envelope to distinct copy (last-admin vs global-admin), falling back to a generic "Could not demote admin."
- [ ] **Task 4 — Tests (AC: #4)**
  ...
  - [ ] Frontend: "Make member" calls `demoteAdmin`; the last-admin and global-admin error codes render their distinct messages.
...
- `frontend/src/api/orgs.ts` (`promoteAdmin` to mirror), `frontend/src/pages/MembersPage.tsx` (`handlePromote`/`handleRemove`, the row actions)
