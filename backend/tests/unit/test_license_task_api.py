"""Tests for the Phase 5 task and the license report endpoint (Story 4.3)."""
...
from generate_sbom.tasks.analysis import classify_licenses
...
_REPORT = {
    "tiers": [{"tier": "Permissive", "packages": [{"name": "django", "version": "3.0", "license": "BSD-3-Clause"}]}],
    "summary": {"Strong Copyleft": 0, "Weak Copyleft": 0, "Unknown": 0, "Permissive": 1},
}
...
def test_phase5_writes_artifact_and_returns_envelope() -> None:
    job = _make_job()
    with patch(_NO_UPDATE), patch("generate_sbom.tasks.analysis.license_service.classify", return_value=_REPORT):
        envelope = classify_licenses.apply(args=({"task_id": str(job.task_id)},)).get()

    artifact_key = f"sbom-results/{job.org_id}/{job.task_id}/licenses.json"
    assert envelope["report_type"] == "license"
    ...
    with default_storage.open(artifact_key) as handle:
        assert json.loads(handle.read())["summary"]["Permissive"] == 1


@pytest.mark.django_db
def test_phase5_failure_returns_failed_envelope() -> None:
    job = _make_job()
    with (
        patch(_NO_UPDATE),
        patch("generate_sbom.tasks.analysis.license_service.classify", side_effect=RuntimeError("pypi down")),
    ):
        envelope = classify_licenses.apply(args=({"task_id": str(job.task_id)},)).get()

    assert envelope["failed"] is True
    assert envelope["failure_reason"] == "license_classification_failed"
    assert not default_storage.exists(f"sbom-results/{job.org_id}/{job.task_id}/licenses.json")


@pytest.mark.django_db
def test_license_report_endpoint_json_inline_and_cross_org_404() -> None:
    job = _make_job()
    artifact_key = f"sbom-results/{job.org_id}/{job.task_id}/licenses.json"
    default_storage.save(artifact_key, ContentFile(json.dumps(_REPORT).encode()))
    AnalysisReport.objects.create(job=job, report_type=AnalysisReport.ReportType.LICENSE, artifact_key=artifact_key)

    ok = _login().get(f"/api/v1/sbom/result/{job.task_id}/reports/licenses/")
    assert ok.status_code == 200
    assert ok.data == _REPORT

    create_org(name="bob", admin_user=register_user(email="bob@example.com", password="pw12345678"))
    cross = _login("bob@example.com").get(f"/api/v1/sbom/result/{job.task_id}/reports/licenses/")
