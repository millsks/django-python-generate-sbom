...
DEBUG = True
# Bandit B104 reads the "0.0.0.0" literal as a bind-all socket address. This is Django's
# ALLOWED_HOSTS (a request Host-header allowlist), not a bind address, and this module is
# the LOCAL-dev settings only — production.py takes ALLOWED_HOSTS from the environment.
# Story 21.1 widened `pixi run security` from the app package to src/, which is why this
# surfaced now. Suppressed on the line itself rather than via [tool.bandit] skips so a
# genuine bind-all elsewhere still fails the gate.
...
# directory under a git-ignored `.celery/broker/` tree (built with pathlib so the paths
