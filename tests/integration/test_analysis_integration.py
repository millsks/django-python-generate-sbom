...
from inventory.tasks.analysis import (
    check_version_currency,
    classify_licenses,
    scan_vulnerabilities,
)
from inventory.tasks.sbom_pipeline import (
...
def _patch_services(stack: ExitStack, *, vuln_fails: bool = False) -> None:
    vuln = stack.enter_context(patch("inventory.analysis.services.vulnerability.scan"))
    if vuln_fails:
        vuln.side_effect = RuntimeError("osv down")
    else:
        vuln.return_value = {"summary": {"vulnerable_package_count": 0, "severity_breakdown": {}}}
    stack.enter_context(
        patch("inventory.analysis.services.license.classify", return_value={"summary": {"Permissive": 1}})
    )
    stack.enter_context(
        patch("inventory.analysis.services.versions.classify", return_value={"summary": {"current": 1}})
    )
