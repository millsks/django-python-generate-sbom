...
from inventory.tasks.analysis import classify_licenses
...
def test_phase5_writes_artifact_and_returns_envelope() -> None:
    job = _make_job()
    with patch(_NO_UPDATE), patch("inventory.tasks.analysis.license_service.classify", return_value=_REPORT):
...
def test_phase5_failure_returns_failed_envelope() -> None:
    ...
    with (
        patch(_NO_UPDATE),
        patch("inventory.tasks.analysis.license_service.classify", side_effect=RuntimeError("pypi down")),
    ):
        envelope = classify_licenses.apply(args=({"task_id": str(job.task_id)},)).get()
