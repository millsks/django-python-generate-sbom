...
from inventory.sbom.parsers import (
    PackageSpec,
    ResolutionError,
    resolve_packages,
    tag_relationships,
)
from inventory.sbom.parsers._pixi import (
    _assert_all_declared_present,
    _solver_problem,
    pixi_lock_from_environment,
)
from inventory.sbom.parsers._uv import parse_compiled, uv_pip_compile
...
def test_requirements_strips_and_uses_uv() -> None:
    with patch("inventory.sbom.parsers.requirements.uv_pip_compile", return_value=_FAKE) as uv:
...
def test_pyproject_extracts_pep621_deps() -> None:
    ...
    with patch("inventory.sbom.parsers.pyproject.uv_pip_compile", return_value=_FAKE) as uv:
...
def test_pixi_toml_extracts_names_excluding_python() -> None:
    ...
    with patch("inventory.sbom.parsers.pixi_toml.uv_pip_compile", return_value=_FAKE) as uv:
...
def test_conda_resolves_via_pixi() -> None:
    ...
    with patch("inventory.sbom.parsers.conda.pixi_lock_from_environment", return_value=_PIXI_LOCK_FROM_ENV) as solve:
...
def test_conda_surfaces_solver_error() -> None:
    ...
    with patch("inventory.sbom.parsers.conda.pixi_lock_from_environment", side_effect=error):
...
def test_uv_pip_compile_runs_subprocess() -> None:
    ...
    with (
        patch("inventory.sbom.parsers._uv.shutil.which", return_value="/bin/uv"),
        patch("inventory.sbom.parsers._uv.subprocess.run", return_value=completed) as run,
    ):
...
def test_uv_pip_compile_empty_returns_empty() -> None:
    with patch("inventory.sbom.parsers._uv.shutil.which", return_value="/bin/uv"):
        assert uv_pip_compile([]) == []


def test_uv_unavailable_raises() -> None:
    with patch("inventory.sbom.parsers._uv.shutil.which", return_value=None):
...
def test_uv_pip_compile_failure_raises() -> None:
    ...
    with (
        patch("inventory.sbom.parsers._uv.shutil.which", return_value="/bin/uv"),
        patch("inventory.sbom.parsers._uv.subprocess.run", side_effect=error),
    ):
        with pytest.raises(ResolutionError):
            uv_pip_compile(["django"])
...
def test_requirements_tags_direct_vs_transitive() -> None:
    with patch("inventory.sbom.parsers.requirements.uv_pip_compile", return_value=_MIXED):
        packages = resolve_packages("requirements", b"django==5.2\n")
...
def test_pyproject_tags_direct_vs_transitive() -> None:
    ...
    with patch("inventory.sbom.parsers.pyproject.uv_pip_compile", return_value=_MIXED):
...
def test_pixi_toml_tags_direct_vs_transitive() -> None:
    ...
    content = b'[dependencies]\nnumpy = ">=1.26"\n'
    with patch("inventory.sbom.parsers.pixi_toml.uv_pip_compile", return_value=resolved):
...
def test_conda_declared_names_skips_invalid_pip_requirement() -> None:
    from inventory.sbom.parsers.conda import _declared_names
...
def test_pixi_lock_from_environment_orchestrates_init_and_lock(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    monkeypatch.setattr("inventory.sbom.parsers._pixi.subprocess.run", _fake_pixi('[dependencies]\nnumpy = "*"\n'))
...
def test_pixi_lock_from_environment_surfaces_solver_problem(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr("inventory.sbom.parsers._pixi.subprocess.run", run)
    with pytest.raises(ResolutionError, match="__cuda"):
