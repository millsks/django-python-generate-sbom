...
def test_soft_timeout_marks_failed_no_artifact() -> None:
    ...
    with (
        patch(_NO_UPDATE),
        patch("inventory.sbom.services.generate_sbom_document", side_effect=SoftTimeLimitExceeded()),
    ):
...
def test_resolution_error_marks_failed_not_stuck() -> None:
    ...
    with (
        patch(_NO_UPDATE),
        patch("inventory.sbom.services.resolve_job_packages", side_effect=ResolutionError("unsatisfiable")),
    ):
...
def test_unexpected_phase_error_falls_back_to_pipeline_error() -> None:
    ...
    with (
        patch(_NO_UPDATE),
        patch("inventory.sbom.services.resolve_job_packages", side_effect=ValueError("boom")),
    ):
        result = pipeline.resolve_transitive_deps.apply(args=(prev,), throw=False)
