...
from inventory.sbom.document import _cyclonedx_license, _spdx_license, _spdx_purl
...
from inventory.sbom.services import finalize_job
from inventory.tasks.sbom_pipeline import generate_sbom_document as generate_phase
from inventory.tasks.sbom_pipeline import persist_artifacts
...
def test_library_error_is_wrapped() -> None:
    with patch("inventory.sbom.generation._generate_cyclonedx", side_effect=ValueError("boom")):
        with pytest.raises(SBOMGenerationError):
...
def _no_license_network() -> Iterator[None]:
    ...
    with patch("inventory.tasks.sbom_pipeline.license_service.build_license_map", return_value={}):
        yield
...
def test_phase3_serializer_error_fails_job_no_artifact() -> None:
    ...
    with (
        patch(_NO_UPDATE),
        patch("inventory.sbom.services.generate_sbom_document", side_effect=SBOMGenerationError("boom")),
    ):
...
def test_phase8_finalizes_job() -> None:
    ...
    from inventory.sbom.services import record_generation
