...
from inventory.tasks.analysis import check_version_currency
...
def test_phase7_writes_artifact_and_returns_envelope() -> None:
    job = _make_job()
    with patch(_NO_UPDATE), patch("inventory.tasks.analysis.versions_service.classify", return_value=_REPORT):
